...
void memModify() {
	...
	for(i = 1; i<=unitSize; i++){
		if(s_address == 0)
		((char*) buffer)[s_address + i - 1] = tmpVal[i-1];
		else 
			*(char*)(s_address + i -1) = tmpVal[i-1];
	}
...
void memDisplay() {
	...
	if (offSet!=0)
	{
		address =(unsigned char*) offSet;
	}
	for (i = 0; i < length; i++) {
		if(unitSize==1){
			printf("%02X ", *(((char*)address) + i) );
		} else if(unitSize==2){
			printf("%04X ", *(((short*)address) + i) );
		} else if(unitSize==4){
...
void Set_Unit_Size() {
	...
	if (strcmp(input, "\n") != 0)
	{
		if ((numberOfUnits == 1) || (numberOfUnits == 2) || (numberOfUnits == 4) ) {

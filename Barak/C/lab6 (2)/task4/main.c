...
int **createPipes(int nPipes) {
	printf("create pipes: %d\n", nPipes);
	int ** myPipes = malloc(sizeof(int*) * nPipes);
	int i;
	for (i = 0; i < nPipes; i++) {
...
void runCommand(cmdLine *line) {
	...
	int ** pipesArray = createPipes(size);
	line = line -> next;
	printf("arg: %s\n", line->arguments[0]);

	int *leftPipeInFD = leftPipe(pipesArray, line);
	if (leftPipeInFD)
		printf("Left pipe out: %d\n", leftPipeInFD[0]);
		printf("Left pipe in: %d\n", leftPipeInFD[1]);
	int *rightPipeInFD = rightPipe(pipesArray, line);
	if (rightPipeInFD){
		printf("Right pipe out: %d\n", rightPipeInFD[0]);
		printf("Right pipe in: %d\n", rightPipeInFD[1]);

	}

	releasePipes(pipesArray, size);

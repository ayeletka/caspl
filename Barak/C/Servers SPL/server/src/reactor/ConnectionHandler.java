...
public class ConnectionHandler<T> {
	...
	protected final WhatsAppTokenizer _tokenizer;
	...
	private ConnectionHandler(SocketChannel sChannel, ReactorData<T> data, SelectionKey key) {
		...
		_tokenizer = _data.getTokenizerMaker().create(true);
		_skey = key;
	}
	...
	/**
	 * adds bytes of the out message to the output stream.
	 * @param buf ByteBuffer contains data from the response message to be sent.
	 */

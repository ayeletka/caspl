...
int main (int argc , char* argv[], char* envp[]){
     ...
     for(j=1; j<argc; j++)
     {
      ...
      if(strcmp(argv[j],"-d")==0) toDebag=1;
     }

 


     fd = system_call(SYS_OPEN, "/home/ayelet/Desktop/workspace/task4/task2b/DIR", O_RDONLY, 0777);
     if(fd <0){
    ...
    debugging(sys_id,itoa(SYS_GETDENTS),toDebag,3);
     ...
     if((onlySuffix==0)  ||  (onlySuffix==1 && (curDir->d_type==8) && strcmp(lastChar,suffix)==0))
      {
